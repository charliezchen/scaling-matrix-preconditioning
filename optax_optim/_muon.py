...
def scale_by_muon(
  ...
  ns_coeffs_array = jnp.asarray(ns_coeffs)
  if ns_coeffs_array.ndim > 2 or ns_coeffs_array.shape[-1] != 3:
      raise ValueError(
          f'ns_coeffs must have shape (3,) or (n, 3), got {ns_coeffs_array.shape}'
      )
  ...
  def update_fn(updates, state, params=None):
    ...
    updates = jax.tree.map(
        lambda x, i, o, n: orthogonalize_via_newton_schulz(
            x, ns_coeffs_array, ns_steps, eps, scale_eps, i, o, n
        ),
        mu_hat,
        din,
        dout,
